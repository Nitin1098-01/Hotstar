class FavouritesController < ApplicationController
  ...
  def create
    @favourite = Favourite.new(:show_id => params[:favourite][:show_id],:user_id => current_user.id)

    respond_to do |format|
      if @favourite.save
        FavouriteMailer.delay(:run_at => @favourite.show.timing - 30.minutes).favourite_email(@favourite)
